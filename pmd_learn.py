...
for epoch in range(repeat):
    ...
    with torch.no_grad(): # 勾配の計算しない
        for image, mask, edge in tqdm(torch.utils.data.DataLoader(valid_dataset,batch_size=1,shuffle=True,num_workers=2)):         
            """問題：モデルの出力値がnanで返ってくる
            img = Image.open(os.path.join(x_train_dir,X_train[idx])) 
            img_label = Image.open(os.path.join(y_train_dir,(os.path.splitext(os.path.basename(X_train[idx]))[0] + '.png')))
            ...
            output = torch.nn.functional.sigmoid(model(img_var)[5]) # 問題：model.eval()だとなぜか０しか返してこないため。
